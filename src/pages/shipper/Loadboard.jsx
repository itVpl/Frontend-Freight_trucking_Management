...
setLoadType(type);
    ...
    // Reset vehicle type when load type changes
    setForm({ ...form, loadType: apiLoadType, vehicleType: '' });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // DRAYAGE loads ke liye additional required fields
    if (form.loadType === 'DRAYAGE') {
      requiredFields.push('returnDate', 'drayageLocation');
    }
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      const payload = {
        ...
        returnLocation: form.drayageLocation || ''
      };
             try {
         const token = localStorage.getItem('token');
