    ...
    const loadApiData = async () => {
      ...
      try {
        ...
        if (!userType || (userType !== "shipper" && userType !== "trucker")) {
          throw new Error(
            "User type not detected. Please login as shipper or trucker.",
          );
        }
        ...
        if (userType === "shipper") {
          ...
          if (!shipperId) {
            throw new Error(
              "Missing shipperId. Please login or pass shipperId via navigation.",
            );
          }
          ...
          if (response.success && response.data.verifiedDOs) {
            transformedData = response.data.verifiedDOs.map((doItem) => {
              ...
              const totalAmount = Number(
                (firstCust?.calculatedTotal ??
                  firstCust?.totalAmount ??
                  lineHaul + fsc + other) ||
                  0,
              );

              const origin =
                lr.origins?.[0] || doItem.shipper?.pickUpLocations?.[0];
              const destination =
                lr.destinations?.[0] || doItem.shipper?.dropLocations?.[0];
              ...
              return {
                ...
                date: lr.pickupDate
                  ? format(new Date(lr.pickupDate), "MM-dd-yyyy")
                  : doItem.date
                    ? format(new Date(doItem.date), "MM-dd-yyyy")
                    : "N/A",
                ...
                destination: destination
                  ? {
                      city:
                        destination.city || destination.extractedCity || "N/A",
                ...
                weight:
                  lr.weight ||
                  doItem.shipper?.pickUpLocations?.[0]?.weight ||
                  "N/A",
          ...
          if (!truckerId) {
            throw new Error(
              "Missing truckerId. Please login or pass truckerId via navigation.",
            );
          }
    ...
    if (reportsData.length === 0) {
      ...
      if (view === "monthly") {
        return {
          ...
          data: [
            5000, 8000, 12000, 18000, 25000, 30000, 37000, 45000, 53000, 60000,
            70000, 77000,
          ],
        };
      } else if (view === "yearly") {
    ...
    <Box sx={{ p: 2 }}>
        ...
        >
          <Box sx={{ mb: 4 }}>
            ...
            >
              ...
              {view === "yearly" &&
                `Yearly revenue comparison (${selectedYear})`}
              {view === "custom" &&
                (startDate && endDate
      ...
      {!loading && !error && (
        <Paper
          elevation={3}
          sx={{
            backgroundColor: themeConfig.content?.bgImage
              ? "rgba(255,255,255,0.94)"
              : themeConfig.table?.bg || "#fff",
            position: "relative",
            ...
            <TableContainer>
              <Table>
                <TableHead>
                  <TableRow
                    sx={{
                      backgroundColor: themeConfig.table?.headerBg || "inherit",
                    }}
                  >
                    ...
                    >
                      ...
                      >
                        <Button
                          ...
                          sx={{
                            backgroundColor:
                              themeConfig.table?.buttonBg || "#1976d2",
          ...
          {selectedConsignment && (
            <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
              {(() => {
                ...
                const customers = Array.isArray(doData?.customers)
                  ? doData.customers
                  : [];
                ...
                const equipmentType =
                  doData?.shipper?.containerType ||
                  lr?.vehicleType ||
                  selectedConsignment.type ||
                  "N/A";
                ...
                return (
                  <>
                    ...
                    {userType === "shipper" ? (
                      ...
                      >
                        ...
                        <Box sx={{ p: 2 }}>
                          ...
                          >
                            <TableBody>
                              {Array.isArray(doData.carrier.carrierFees) &&
                                doData.carrier.carrierFees.map((fee, idx) => (
                                  <TableRow key={idx}>
                                    <TableCell
                                      sx={{
                                        width: 220,
                              ...
                              <TableRow>
                                ...
                                >
                                  $
                                  {Number(
                                    doData.carrier.totalCarrierFees || 0,
                                  ).toLocaleString()}
                                </TableCell>
                              </TableRow>
                            </TableBody>
                        ...
                        >
                          <TableBody>
                            <TableRow>
                              ...
                              <TableCell sx={{ fontWeight: 600 }}>
                                {lr?.weight ??
                                  doData?.shipper?.pickUpLocations?.[0]
                                    ?.weight ??
                    ...
                    {/* Locations Card */}
                    {(Array.isArray(doData?.shipper?.pickUpLocations) ||
                      Array.isArray(doData?.shipper?.dropLocations) ||
                      lr?.origins ||
                      lr?.destinations) && (
                      <Paper
                        elevation={0}
                      ...
                      >
                        ...
                        <Box sx={{ p: 2 }}>
                          {(
                            doData?.shipper?.pickUpLocations ||
                            lr?.origins ||
                            []
                          ).map((l, idx) => (
                            <Box
                          ...
                          <TableBody>
                            ...
                            <TableRow>
                              ...
                              <TableCell sx={{ fontWeight: 600 }}>
                                {lr?.deliveryDate
                                  ? format(
                                      new Date(lr.deliveryDate),
                                      "yyyy-MM-dd",
                                    )
                                  : doData?.shipper?.dropLocations?.[0]
                                        ?.dropDate
                                    ? format(
                                        new Date(
                                          doData.shipper.dropLocations[0]
                    ...
                    {(() => {
                      ...
                      const pickupGroups = [
                        {
                          title: "Empty Truck Images",
                          urls:
                            lr?.emptyTruckImages ||
                            doData?.emptyTruckImages ||
                            [],
                        },
                        {
                          title: "Loaded Truck Images",
                          urls:
                            lr?.loadedTruckImages ||
                            doData?.loadedTruckImages ||
                            [],
                        },
                        {
                          ...
                          urls:
                            lr?.containerImages ||
                            doData?.containerImages ||
                            [],
                        },
                        {
                          title: "Seal Images",
                          urls: lr?.sealImages || doData?.sealImages || [],
                        },
                        {
                          title: "EIR Tickets",
                          urls: lr?.eirTickets || doData?.eirTickets || [],
                        },
                      ].filter(
                        (g) => Array.isArray(g.urls) && g.urls.length > 0,
                      );
                      ...
                      const dropGroups = [
                        ...
                        {
                          title: "Loaded Truck Images (Drop)",
                          urls: lrImgs?.loadedTruckImages || [],
                        },
                        {
                          title: "Drop Location Images",
                          urls: lrImgs?.dropLocationImages || [],
                        },
                        {
                          title: "Empty Truck Images (Drop)",
                          urls: lrImgs?.emptyTruckImages || [],
                        },
                      ].filter(
                        (g) => Array.isArray(g.urls) && g.urls.length > 0,
                      );
                      ...
                      if (pickupGroups.length === 0 && dropGroups.length === 0)
                        return null;
