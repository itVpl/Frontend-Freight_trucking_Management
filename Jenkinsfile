...
pipeline {
  ...
  parameters {
    string(name: 'FRONTEND_DIR', defaultValue: '.', description: 'Frontend folder')
    booleanParam(name: 'LIGHT_BUILD', defaultValue: true, description: 'Light build')
    string(name: 'NODE_HEAP_MB', defaultValue: '2048', description: 'Node heap MB')
  }
      ...
      steps {
        sh '''
          ...
          if [ -d "$WORKSPACE/${FRONTEND_DIR}/dist" ]; then
            ...
            echo "Build output folder missing."
            exit 1
          fi
          ...
          echo "Deployed: $(readlink -f $APP_DIR/current)"
        '''
      }
    }
  }
